...
{
    public class MissingPetPostDTOValidator : AbstractValidator<MissingPetPostPutDTO>
    {
        public MissingPetPostDTOValidator()
        {
            ...
            RuleFor(x => x.MissingDate).NotNull().NotEmpty().WithMessage("Kayıp olma tarihi kısmı boş veya null olamaz.");
            RuleFor(x => x.MissingDate).Must(BeValidMissingDate).WithMessage("Kayıp olma tarihi gelecekte olamaz.");

            RuleFor(x => x.LastSeenAddress).NotEmpty().WithMessage("En son görülme kısmı boş olamaz. Hatırlamıyorsanız lütfen null geçin.");
